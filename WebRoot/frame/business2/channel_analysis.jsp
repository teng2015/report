...
<div class="portlet box blue">
	...
	<div class="portlet-body flip-scroll">
		<div style="text-align: right;">
			渠道类型：
			<wl:dwrite dictType="CHANNEL_TYPE_NAME" id="channelTypeName" style="height:20px;padding:0px;margin-bottom: 4px;width:110px" nullStr="全部"/>
			渠道名称:
			<input type="text" id="channelName" style="width:110px;height:18px;padding: 2px;margin-bottom:4px;line-height:normal;">
			媒介：
			<wl:dwrite dictType="MEDIUM_NAME" id="mediumName" style="height:20px;padding:0px;margin-bottom: 4px;width:110px" nullStr="全部"/>
...
function downExcelFile(){
	...
	if($("#channelTypeName").val()!=''){
		rtpparams.channel_type = 'AND CHANNEL_TYPE = '+$("#channelTypeName").val();
	}
	rtpparams.channel_name = '%'+$('#channelName').val()+'%';
	if($("#mediumName").val()!=''){
		rtpparams.medium_id = 'AND MEDIUM_ID = '+$("#mediumName").val();
	}
	RptManager.genReportFile(rtpparams,'channel1.rptdisger',{
...
//跳转到渠道详细
function skipDetail(channelId,channelName){
...
function querygirdPartPage(currentPage){
	...
	//通过ajax查询后台数据
	RptManager.genReportDataToHtmlAndCount(rtpparams,'channel1.rptdisger', function(content){
		...
		param.tdw=['60px','140px','70px','50px','60px','85px','60px','85px','70px','85px','140px','140px'];
		showgirdbirt01(param);
	});
}
$(document).ready(function() {
	if("${dataTime}"!=''){
		$('#startTime1').val("${dataTime}");
	}
	if('${mediumName}'!=''){
		$('#mediumName').val('${mediumName}');
	}
     parampage = new page();
