...
<div class="portlet box blue">
	...
	<div class="portlet-body flip-scroll">
		<div style="text-align: right;">
			游戏ID:
			<input type="text" id="productId" style="width:110px;height:18px;padding: 2px;margin-bottom:4px;line-height:normal;">
			游戏名称:
			<input type="text" id="appName" style="width:110px;height:18px;padding: 2px;margin-bottom:4px;line-height:normal;">
			商务：
...
function downExcelFile(){
	...
	rtpparams.product_id = '%'+$('#productId').val()+'%';
	rtpparams.app_name = '%'+$('#appName').val()+'%';
	if($("#commerceName").val()!=''){
		rtpparams.commerce_id = 'AND COMMERCE_ID = '+$("#commerceName").val();
	}
	RptManager.genReportFile(rtpparams,'order1.rptdisger',{
...
//跳转到订单详细
function skipDetail(orderId,appName){
	var loadURL = '/frame/business2/order_detail_analysis.jsp?';
	var urlParam = "orderId="+orderId+"&appName="+appName;
	jQuery.ajax({
		type : "POST",
...
function querygirdPartPage(currentPage){
	...
	//通过ajax查询后台数据
	RptManager.genReportDataToHtmlAndCount(rtpparams,'order1.rptdisger', function(content){
		parampage.count=parseInt(content[1]);
		//构造报表模板参数，并展示模板
		var param = new girdbirt01param(content[0],parampage);
		param.showLink={0:{name:'skipDetail',arg:[0,2]}};
