...
function downExcelFile(){
	...
	if($("#mediumName").val()!=''){
		rtpparams.commerce_id = 'AND MEDIUM_ID = '+$("#mediumName").val();
	}
	RptManager.genReportFile(rtpparams,'medium1.rptdisger',{
...
//跳转到订单详细
function skipDetail(mediumId,mediumName){
	var loadURL = '/frame/business2/medium_detail_analysis.jsp?';
	var urlParam = "mediumId="+mediumId+"&mediumName="+mediumName;
	jQuery.ajax({
		type : "POST",
...
function querygirdPartPage(currentPage){
	...
	//通过ajax查询后台数据
	RptManager.genReportDataToHtmlAndCount(rtpparams,'medium1.rptdisger', function(content){
