...
<div class="portlet box blue">
	<div class="portlet-title">
		<div class="caption" style="height: 20px;">
			<i class="icon-reorder"></i>商务明细-><span id="commerceName"></span>
		</div>
		<div class="tools">
...
function downExcelFile(){
	...
	rtpparams.commerce_id = commerceId;
	rtpparams.start_index = 0;
	rtpparams.end_index = 100
	RptManager.genReportFile(rtpparams,'commerce2.rptdisger',{
...
function querygirdPartPage(currentPage){
	...
	//通过ajax查询后台数据
	RptManager.genReportDataToHtmlAndCount(rtpparams,'commerce2.rptdisger', function(content){
...
$(document).ready(function() {
	if('${sessionScope.userInfo.commerceId}'!='-1'){
		$('#commerceName').html('${sessionScope.userInfo.name}');
		commerceId='${sessionScope.userInfo.commerceId}';
	}
	if('${param.commerceName}'!=''){
		$('#commerceName').html('${param.commerceName}');
	}
	if('${param.commerceId}'!=''){
		commerceId = '${param.commerceId}';
	}
    parampage = new page();
